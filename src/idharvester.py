            ...
            while len(collected_ids) < target_count:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if not ids_found:
                            tqdm.write("No more games found. Steam may cut off search results at this page limit.")
                            break
                        ...
                        tqdm.write(f"  -> Page {page}: Found {len(ids_found)} IDs ({new_added} new). Total: {len(collected_ids)} / {target_count}")
                        ...
                        tqdm.write("  -> ERROR 429: Rate limited! Sleeping for 60 seconds...")
                        time.sleep(60)
                        continue
                        
                    else:
                        tqdm.write(f"Failed to load page {page}. Status code: {response.status_code}")
                        break
